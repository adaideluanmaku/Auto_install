...
public class Mm_config {
	...
	public void resources() {
		...
		try {
			...
			file=new File(path+"passrhmm\\WEB-INF\\classes\\application.properties");
...
//			System.out.println(pro.getProperty("jdbc.driver"));
			...
			pro.setProperty("passaa.path", files_config.getPassaa_path());
			pro.setProperty("passcore.path", files_config.getPasscore_path());
//			pro.setProperty("passtcm.path", files_config.getPasstcm_path());
	...
	public boolean config(Files_config files_config){
		...
		//1清空作废压缩包文件夹
		try {
			filesHandle.findfiles(path+"passrhmm\\");
			filesHandle.createdir(path+"passrhmm\\");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		//0.解压文件
		if(!antWarUtils.unzip(path+"passrhmm.war", path+"passrhmm\\", false)){
			return false;
		}
		...
		//压缩文件
		antWarUtils.zip(path+"passrhmm\\", path+"passrhmm_new.war");
		return true;
	}
}
